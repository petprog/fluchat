  ...
  Widget build(BuildContext context) {
    return PagedValueListenableBuilder<int, Channel>(
      ...
      builder: (context, value, child) {
        return value.when(
          (channels, nextPageKey, error) => channels.isNotEmpty
              ? CustomScrollView(
                  slivers: [
...
class _MessageTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        // Navigator.of(context).push(ChatScreen.route(channel));
      },
      child: Container(
    ...
    return BetterStreamBuilder<DateTime>(
      ...
      builder: (context, data) {
        ...
        if (lastMessageAt.millisecondsSinceEpoch >=
          ...
          stringDate = 'YESTERDAY, ${Jiffy(lastMessageAt.toLocal()).jm}';
        } else if (startOfDay.difference(lastMessageAt).inDays < 7) {
          stringDate = Jiffy(lastMessageAt.toLocal()).EEEE;
        } else {
          stringDate = Jiffy(lastMessageAt.toLocal()).yMEd;
        }
        return Text(
          stringDate,
          style: const TextStyle(
